...
export const registerVendor = async (req, res) => {
  try {
    ...
    res.status(400).json({
      status: 400,
      message: `error occured while registering user ${e}`,
    });
  }
};
...
export const loginVendor = async (req, res) => {
  try {
    ...
    if (user && (await user.matchPassword(password))) {
      // the matchPassword here is created in userModal.js where we have checked it with password_hash of individual user.
      res.status(200).json({
        user: {
          _id: user._id,
          name: user.full_name,
...
export const registerUser = async (req, res) => {
  try {
    ...
    if (!contact || !name)
      return res
        .status(400)
        .json({ status: 400, message: "contact detail is required" });

    const existingContact = Users.findOne({ contact: contact });
    if (!existingContact)
      return res
        .status(400)
        .json({ status: 400, message: "contact not found." });
    ...
    res
      .status(500)
      .json({ message: `error while sending otp due to ${e.message}` });
  }
};
...
export const verifyOtp = async (req, res) => {
  try {
    ...
    if (!contact || !otp)
      return res
        .status(400)
        .json({ status: 400, message: "Contact & Otp is required" });
    ...
    if (!otpRecord)
      return res
        .status(400)
        .json({ status: 400, message: "OTP expired or not found" });

    // check expiry in 5 min
    const isExpired =
      new Date().getTime() - otpRecord.createdAt.getTime() > 1 * 60 * 1000;
    if (isExpired) {
    ...
    return res
      .status(500)
      .json({ msg: "Error verifying OTP", error: e.message });
  }
};
